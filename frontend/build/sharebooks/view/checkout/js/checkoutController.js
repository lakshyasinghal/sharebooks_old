...
checkoutApp.controller("CheckoutController" , ["$scope" , "$http" , function($scope , $http){
	...
	$scope.bookRequest = function(){
		try{
			...
			var params = $scope.getBookRequestParams();
			var paramString = getParamString(params);

			$http({
				url: urls.ADD_BOOK_REQUEST + "?" + paramString,
				method: "GET",
			...
			$scope.showError("Error in bookRequest --- " + err.message);
		}
	};


	$scope.getBookRequestParams = function(){
		try{
			var params = {};
			//params.id = -1;
			...
			$scope.showError("Error in getBookRequestParams --- " + err.message);
		}
	};
