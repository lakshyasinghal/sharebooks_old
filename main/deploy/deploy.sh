...
sourcePath="$applicationPath/main/webapp"

libPath="$sourcePath/lib"

buildPath="$applicationPath/frontend/build"
appName="sharebooks"
tomcatPath="/Users/lakshya.singhal/Desktop/tomcat1/webapps"
...
dependency[0]="javax.mail.jar"
dependancy[1]="mysql-connector-java-5.1.40-bin.jar"
dependancy[2]="javax.servlet.jar"
...
function javaCompile {
	...
	javac -d $classesInitialPath -classpath $classesInitialPath:$libPath/${dependancy[0]}:$libPath/${dependancy[1]}:$libPath/${dependancy[2]} $1/*.java
	echo
}
...
function moveBuild {
	...
	echo
	echo "moving $appName app to tomcat1 webapps folder"
	echo
